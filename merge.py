...
def BuildHanguelTranslit(font, latinNames):
    from rrk import HanguelTranslit
    cmap_ = font["cmap"]
    glyf_ = font["glyf"]
    isCff = "CFF_" in font
    latinGlyphs = { k: glyf_[name] for k, name in latinNames.items() }

    underlineWidths = set()
    for ch in range(0xAC00, 0xD7A4):
        name = cmap_.get(str(ch))
        if not name:
            continue
        glyph = glyf_.get(name)
        advanceWidth = glyph["advanceWidth"]
        underlineLeft = advanceWidth
        references = [{"glyph": name, "x": 0, "y": 0, "a": 1, "b": 0, "c": 0, "d": 1}]
        for letter in HanguelTranslit(ch):
            references.append({"glyph": latinNames[letter], "x": advanceWidth, "y": 0, "a": 1, "b": 0, "c": 0, "d": 1})
            advanceWidth += latinGlyphs[letter]["advanceWidth"]
        underlineWidth = advanceWidth - underlineLeft
        underlineWidths.add(underlineWidth)
        references.append({
            "glyph": ".rrkUnderline.{}".format(underlineWidth),
            "x": underlineLeft, "y": 0, "a": 1, "b": 0, "c": 0, "d": 1,
        })
        cmap_[str(ch)] = name + ".rrkHanguelTranslit"
        if isCff:
            glyf_[name + ".rrkHanguelTranslit"] = Dereference({
                "advanceWidth": advanceWidth,
                "references": references,
            }, font)
        else:
            glyf_[name + ".rrkHanguelTranslit"] = {
                "advanceWidth": advanceWidth,
                "references": references,
            }
...
if __name__ == '__main__':
    ...
    # Hanguel romanization
    if "RRK" in param["feature"]:
        with open("build/rrk/{}.otd".format(configure.GenerateFilename(dep['RRK'])), 'rb') as rrkFile:
            rrkFont = json.loads(rrkFile.read().decode('UTF-8', errors='replace'))
        rrkLatinList = "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
        rrkLatinNames = {ch.lower(): rrkFont['cmap'][str(ord(ch))] for ch in rrkLatinList}
        MergeBelow(baseFont, rrkFont)
        BuildHanguelTranslit(baseFont, rrkLatinNames)
